...
$delId=$_GET['delete'];
$del_service=mysqli_query($conn, "DELETE FROM surmons WHERE id=$delId");
}
?>
...
$id=$delId[$j];
$del_service=mysqli_query($conn, "DELETE FROM surmons WHERE id=$id");
}
...
<div class="col-lg-12 table-responsive">
    ...
    <form action="" method="post">
      <div class="col-xl-6 m-3"><button name="deleteService" type="submit" class="col-xl-3 btn btn-danger" onClick="return confirm('Do you want to delete the selected Services')" >Delete</button>
        <a href="index.php?source=add" class="col-xl-3 btn text-light btn-warning ">Add Surmon</a></div>
    ...
    <tr>
        <?php 
    $get_service = mysqli_query($conn, "SELECT * FROM surmons");
    if(isset($_GET['page'])){
        $page = $_GET['page'];
    } else {
        $page = 1;
    ...
    $per_page = mysqli_query($conn, "SELECT * FROM surmons ORDER by id DESC LIMIT $startPage, $pageShow");
    $i=$reverse;
